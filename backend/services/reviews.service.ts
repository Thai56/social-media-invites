...
const handleEmail = (place_id: string, reviews: any[], data: string) => {
...
const handleReviews = (place_id: string, reviews: any[]) => {
  rp(`${GOOGLE_PLACE_URL}?placeid=${place_id}&${FIELDS}&key=${SECRET_KEY}`)
    .then((data: string) => handleEmail(place_id, reviews, data),
      (err) => console.log(
        'error fetching the review for business ',
        err,
        place_id
      )
    )
...
const reviewHandler = () => {
  ...
  return businessDbInstance => {
    ...
    businessDbInstance.find({}).toArray((err, result) => {
      if (err) console.error('There was an error getting all businesses', err);
